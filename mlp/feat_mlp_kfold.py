...
def score_kfold(splits):
    ...
    for train_i,test_i in kfold.split(x_inter):
        ...
        # tuned MLP for galactic class using hyper-param tuning
        model = MLPClassifier(
            alpha = 10**(-6),
            activation = 'tanh',
            solver = 'adam',
    ...
    for train_i,test_i in kfold.split(x_extra):
        ...
        # it has been tested, and scaling all features yeilds better results for MLP
        scale = StandardScaler()
        scale.fit(x_train)
        x_train = pd.DataFrame(scale.transform(x_train), columns=x_train.columns)
        x_test  = pd.DataFrame(scale.transform(x_test), columns=x_test.columns)

        # tuned MLP for extragalactic class using hyper-param tuning
        model = MLPClassifier(
            alpha = 10**(-3),
            activation = 'relu',
            solver = 'adam',
